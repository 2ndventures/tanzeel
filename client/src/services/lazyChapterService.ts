...
class LazyChapterService {
  ...
  private async fetchScriptText(chapterId: number, script: ArabicScript): Promise<{ verse_key: string; text: string; words?: string[] }[]> {
    // Try the bundled static JSON first — these ship with the app so the
    // iPhone build works fully offline and without any backend at all.
    // (Generated by scripts/generate-script-data.mjs.)
    const bundledUrl = `/data/chapters-${script}/${chapterId}.json`;
    try {
      const bundledRes = await fetch(bundledUrl);
      if (bundledRes.ok) {
        const data = await bundledRes.json();
        if (data?.verses?.length) return data.verses;
      }
    } catch {
      // Fall through to the network fetch below.
    }

    // Network fallback: only useful in dev or if a bundled file is missing.
