...
import { migrateUpdatedAt, migrateUserId } from "@/lib/bookmarkService";
...
function App() {
  ...
  useEffect(() => {
    ...
    const FIRE_DELAY_MS = 50;
    const SCROLL_CAPTURE: AddEventListenerOptions = { passive: true, capture: true };
    ...
    const onMove = (e: PointerEvent) => {
      if (pointerId === null || e.pointerId !== pointerId || fired) return;
    ...
    window.addEventListener('scroll', onScroll, SCROLL_CAPTURE);
    ...
    return () => {
      ...
      window.removeEventListener('scroll', onScroll, SCROLL_CAPTURE);
    };
  }, []);
  ...
  useEffect(() => {
    try {
      localStorage.removeItem("tanzeel:devtool:active-word-color");
      localStorage.removeItem("tanzeel:devtool:active-word-glow");
