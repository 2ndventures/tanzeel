...
CHAPTERS_DIR = "public/data/chapters"
OUTPUT_FILE = "verses_export.csv"

# Canonical verse counts per surah (standard Hafs count)
...
with open(OUTPUT_FILE, "w", newline="", encoding="utf-8") as csvfile:
    ...
    writer.writerow(["surah_id", "verse_number", "arabic_text", "transliteration", "translation"])

    for surah_id in range(1, 115):
        path = os.path.join(CHAPTERS_DIR, f"{surah_id}.json")
        ...
        expected = EXPECTED_VERSE_COUNTS.get(surah_id)
        if expected is not None and len(verses) != expected:
            mismatches.append((surah_id, len(verses), expected))

print(f"Total verses exported: {total_verses}")
print(f"Expected total: 6236")
if total_verses != 6236:
    print(f"  WARNING: count mismatch! Got {total_verses}, expected 6236.")

if mismatches:
    print("\nSurahs with unexpected verse counts:")
    for sid, got, exp in mismatches:
        print(f"  Surah {sid}: got {got}, expected {exp}")
else:
    print("All surah verse counts match expected values.")
